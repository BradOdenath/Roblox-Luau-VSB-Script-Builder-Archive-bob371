...
local configure_left_arm = function()
	...
	local lm = liggamentize(
		...
		"Left Shoulder",	--Name	
		CFrame.new(
			-sizex - la.Size.x/2 +1, 	--x
			(la.Size.y/4)-2,		--y
			-0.5*scale		-((1-scale)*0.25)					--z
...
local body_part_character = function()
	local right_arm = body_part(Vector3.new(1,2,1),Color3.fromRGB(123,182,232),'Ryght Arm')
	local lm = liggamentize(
		right_arm, 			--limb
		torso,				--attached_to	
		"Motor",			--Type	
		"Right Shoulder",	--Name	
		CFrame.new(
			sizex - right_arm.Size.x/2 +1 + (1-scale), 		--x
	...
	local left_arm = body_part(Vector3.new(1,2,1),Color3.fromRGB(123,182,232),'Lhft Arm')
	local lm = liggamentize(
		left_arm,			--limb
		torso,				--attached_to	
		"Motor",			--Type	
		"Lhft Shoulder",	--Name	
		CFrame.new(
			-sizex - left_arm.Size.x/2 - (1-scale), 	--x
			(left_arm.Size.y/4),			--y
			0						--z
			) * CFrame.fromEulerAnglesXYZ(
				0,						--x
	...
	local nose = body_part(Vector3.new(0.2,0.4,0.2),Color3.fromRGB(123,182,232),'Nose')
	local lm = liggamentize(
		nose,				--limb
		torso,				--attached_to	
		"Motor",			--Type	
...
local configure_character = function()
	...
	polish_character()
	body_part_character()
	color_character(Color3.fromRGB(123,182,232))
	torso.Color = Color3.fromRGB(105,64,40)
	reanimate()
end
